...
RUN echo -e "[bahmni] \nname=Bahmni development repository for RHEL/CentOS 6\nbaseurl=http://repo.mybahmni.org/rpm/bahmni/\nenabled=1\ngpgcheck=0\n" > /etc/yum.repos.d/bahmni.repo \
    ...
    && yum -y install epel-release python-pip git sudo wget ansible openssh-server openssh-clients tar yum-plugin-ovl bahmni-installer-0.90-308 java-1.8.0-openjdk-devel Xvfb firefox \
    && pip install awscli \
    && pip install boto \
